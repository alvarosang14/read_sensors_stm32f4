...
/** @defgroup PWREx_Regulator_state_in_UnderDrive_mode PWREx Regulator state in
 * UnderDrive mode
 * @{
 */
#define PWR_MAINREGULATOR_UNDERDRIVE_ON PWR_CR_MRUDS
#define PWR_LOWPOWERREGULATOR_UNDERDRIVE_ON                                    \
    ((uint32_t)(PWR_CR_LPDS | PWR_CR_LPUDS))
/**
 * @}
 */
...
#define PWR_REGULATOR_VOLTAGE_SCALE2                                           \
    PWR_CR_VOS_1 /* Scale 2 mode: the maximum value of fHCLK is 144 MHz. It    \
                    can be extended to 168 MHz by activating the over-drive                                          \
                    mode. */
 ...
 * @}
 */
#endif /* STM32F410xx || STM32F446xx || STM32F412Zx || STM32F412Vx ||          \
 ...
 *         a tradeoff between performance and power consumption when the device
 * does not operate at the maximum frequency (refer to the datasheets for more
 * details). This parameter can be one of the following values:
 *            @arg PWR_REGULATOR_VOLTAGE_SCALE1: Regulator voltage output Scale
 * 1 mode
 *            @arg PWR_REGULATOR_VOLTAGE_SCALE2: Regulator voltage output Scale
 * 2 mode
 * @retval None
 */
 ...
 *            @arg PWR_REGULATOR_VOLTAGE_SCALE2: Regulator voltage output Scale
 * 2 mode
 *            @arg PWR_REGULATOR_VOLTAGE_SCALE3: Regulator voltage output Scale
 * 3 mode
 * @retval None
 */
#define __HAL_PWR_VOLTAGESCALING_CONFIG(__REGULATOR__)                         \
...
#define __HAL_PWR_OVERDRIVESWITCHING_ENABLE()                                  \
    (*(__IO uint32_t *)CR_ODSWEN_BB = ENABLE)
#define __HAL_PWR_OVERDRIVESWITCHING_DISABLE()                                 \
    (*(__IO uint32_t *)CR_ODSWEN_BB = DISABLE)

/** @brief Macros to enable or disable the Under drive mode.
 * @note  This mode is enabled only with STOP low power mode.
 *        In this mode, the 1.2V domain is preserved in reduced leakage mode.
 * This mode is only available when the main regulator or the low power
 * regulator is in low voltage mode.
 * @note  If the Under-drive mode was enabled, it is automatically disabled
 * after exiting Stop mode. When the voltage regulator operates in Under-drive
 * mode, an additional startup delay is induced when waking up from Stop mode.
 */
 ...
 *                                 is ready
 *            @arg PWR_FLAG_ODSWRDY: This flag indicates that the Over-drive
 * mode switching is ready
 *            @arg PWR_FLAG_UDRDY: This flag indicates that the Under-drive mode
 *                                 is enabled in Stop mode
 * @retval The new state of __FLAG__ (TRUE or FALSE).
 */
#define __HAL_PWR_GET_ODRUDR_FLAG(__FLAG__)                                    \
    ((PWR->CSR & (__FLAG__)) == (__FLAG__))
...
/** @defgroup PWREx_IS_PWR_Definitions PWREx Private macros to check input
 * parameters
 * @{
 */
#if defined(STM32F427xx) || defined(STM32F437xx) || defined(STM32F429xx) ||    \
