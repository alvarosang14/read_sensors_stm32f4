...
#define LL_PWR_REGU_VOLTAGE_SCALE1                                             \
    (PWR_CR_VOS_0 |                                                            \
     PWR_CR_VOS_1) /* The SCALE1 is not available for STM32F401xx devices */
#else
...
#define LL_PWR_MODE_STOP_MAINREGU_UNDERDRIVE                                   \
    (PWR_CR_MRUDS |                                                            \
     PWR_CR_FPDS) /*!< Enter Stop mode (with main Regulator in under-drive     \
                     mode) when the CPU enters deepsleep */
#define LL_PWR_MODE_STOP_LPREGU_UNDERDRIVE                                     \
    (PWR_CR_LPDS | PWR_CR_LPUDS |                                              \
     PWR_CR_FPDS) /*!< Enter Stop mode (with low power Regulator in            \
                     under-drive mode) when the CPU enters deepsleep */
...
#define LL_PWR_MODE_STOP_MAINREGU_DEEPSLEEP                                    \
    (PWR_CR_MRLVDS |                                                           \
     PWR_CR_FPDS) /*!< Enter Stop mode (with main Regulator in Deep Sleep      \
                     mode) when the CPU enters deepsleep */
#define LL_PWR_MODE_STOP_LPREGU_DEEPSLEEP                                      \
    (PWR_CR_LPDS | PWR_CR_LPLVDS |                                             \
     PWR_CR_FPDS) /*!< Enter Stop mode (with low power Regulator in Deep Sleep \
                     mode) when the CPU enters deepsleep */
...
#define LL_PWR_REGU_DSMODE_LOW_POWER                                           \
    (PWR_CR_LPDS) /*!< Voltage Regulator in low-power mode during deepsleep    \
                     mode */
/**
 * @}
 */
...
#endif                                     /* PWR_CSR_EWUP1 */
#if defined(PWR_CSR_EWUP2)
#define LL_PWR_WAKEUP_PIN2                                                     \
    (PWR_CSR_EWUP2) /*!< WKUP pin 2 : PC0 or PC13 according to device */
#endif              /* PWR_CSR_EWUP2 */
#if defined(PWR_CSR_EWUP3)
...
LL_PWR_IsEnabledLowPowerRegulatorDeepSleepUDMode(void) {
 ...
 *         (*) LL_PWR_REGU_VOLTAGE_SCALE1 is not available for STM32F401xx
 * devices
 * @retval None
 */
 ...
 * @note The BRE bit of the PWR_CSR register is protected against parasitic
 * write access. The LL_PWR_EnableBkUpAccess() must be called before using this
 * API.
 * @retval None
 */
...
__STATIC_INLINE void LL_PWR_SetPowerMode(uint32_t PDMode) {
    ...
    MODIFY_REG(PWR->CR,
               (PWR_CR_PDDS | PWR_CR_LPDS | PWR_CR_FPDS | PWR_CR_LPLVDS |
                PWR_CR_MRLVDS),
               PDMode);
#else
...
#if defined(PWR_CR_MRUDS) && defined(PWR_CR_LPUDS) && defined(PWR_CR_FPDS)
    return (
        uint32_t)(READ_BIT(PWR->CR, (PWR_CR_PDDS | PWR_CR_LPDS | PWR_CR_FPDS |
                                     PWR_CR_LPUDS | PWR_CR_MRUDS)));
#elif defined(PWR_CR_MRLVDS) && defined(PWR_CR_LPLVDS) && defined(PWR_CR_FPDS)
    return (
        uint32_t)(READ_BIT(PWR->CR, (PWR_CR_PDDS | PWR_CR_LPDS | PWR_CR_FPDS |
                                     PWR_CR_LPLVDS | PWR_CR_MRLVDS)));
#else
