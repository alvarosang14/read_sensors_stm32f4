                    ...
                    if ((epint & USB_OTG_DIEPINT_XFRC) ==
                        ...
                        if (hpcd->Init.dma_enable == 1U) {
                            ...
                            if ((epnum == 0U) &&
                                ...
                                (void)USB_EP0_OutStart(hpcd->Instance, 1U,
                                                       (uint8_t *)hpcd->Setup);
                            }
                        }
...
static HAL_StatusTypeDef PCD_WriteEmptyTxFifo(PCD_HandleTypeDef *hpcd,
    ...
    while (
        ...
        (void)USB_WritePacket(USBx, ep->xfer_buff, (uint8_t)epnum,
                              (uint16_t)len, (uint8_t)hpcd->Init.dma_enable);
...
static HAL_StatusTypeDef PCD_EP_OutXfrComplete_int(PCD_HandleTypeDef *hpcd,
    ...
    if (hpcd->Init.dma_enable == 1U) {
        if ((DoepintReg & USB_OTG_DOEPINT_STUP) ==
            USB_OTG_DOEPINT_STUP) /* Class C */
        {
            /* StupPktRcvd = 1 this is a setup packet */
            if ((gSNPSiD > USB_OTG_CORE_ID_300A) &&
                ((DoepintReg & USB_OTG_DOEPINT_STPKTRX) ==
                 USB_OTG_DOEPINT_STPKTRX)) {
                CLEAR_OUT_EP_INTR(epnum, USB_OTG_DOEPINT_STPKTRX);
            }
        } else if ((DoepintReg & USB_OTG_DOEPINT_OTEPSPR) ==
                   USB_OTG_DOEPINT_OTEPSPR) /* Class E */
        {
            CLEAR_OUT_EP_INTR(epnum, USB_OTG_DOEPINT_OTEPSPR);
        } else if ((DoepintReg &
                    (USB_OTG_DOEPINT_STUP | USB_OTG_DOEPINT_OTEPSPR)) == 0U) {
            /* StupPktRcvd = 1 this is a setup packet */
            if ((gSNPSiD > USB_OTG_CORE_ID_300A) &&
                ((DoepintReg & USB_OTG_DOEPINT_STPKTRX) ==
                 USB_OTG_DOEPINT_STPKTRX)) {
                ...
                ep->xfer_count = ep->xfer_size - (USBx_OUTEP(epnum)->DOEPTSIZ &
        ...
        if (gSNPSiD == USB_OTG_CORE_ID_310A) {
            ...
            if ((epnum == 0U) && (hpcd->OUT_ep[epnum].xfer_len == 0U)) {
                /* this is ZLP, so prepare EP0 for next setup */
                (void)USB_EP0_OutStart(hpcd->Instance, 0U,
                                       (uint8_t *)hpcd->Setup);
            }
