...
#define UFB_MODE_BB                                                            \
    (uint32_t)(PERIPH_BB_BASE + (MEMRMP_OFFSET * 32U) +                        \
               (UFB_MODE_BIT_NUMBER * 4U))
...
#define CMPCR_CMP_PD_BB                                                        \
    (uint32_t)(PERIPH_BB_BASE + (CMPCR_OFFSET * 32U) + (CMP_PD_BIT_NUMBER * 4U))
...
#define MCHDLYCR_BSCKSEL_BB                                                    \
    (uint32_t)(PERIPH_BB_BASE + (MCHDLYCR_OFFSET * 32U) +                      \
               (BSCKSEL_BIT_NUMBER * 4U))
/**
 * @}
 */
 ...
 * instruction and Data caches. Configures the SysTick to generate an interrupt
 * each 1 millisecond, which is clocked by the HSI (at this stage, the clock is
 * not yet configured and thus the system is running from the internal HSI at 16
 * MHz). Set NVIC Group Priority to 4. Calls the HAL_MspInit() callback function
 * defined in user file "stm32f4xx_hal_msp.c" to do the global low level
 ...
 * systick. This function is optional.
 * @retval HAL status
 */
 ...
 *
 * @note   This function can be used only for STM32F42xxx/43xxx/469xx/479xx
 * devices.
 *
 ...
 * @0x00000000) and Flash Bank2 mapped at 0x08100000 (and aliased at 0x00100000)
 *
 * @retval None
 */
