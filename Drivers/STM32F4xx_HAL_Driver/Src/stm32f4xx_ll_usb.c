...
/** @defgroup USB_LL_Exported_Functions_Group1 Initialization/de-initialization
functions
...
HAL_StatusTypeDef USB_SetTurnaroundTime(USB_OTG_GlobalTypeDef *USBx,
    ...
    frequency used by application. In the low AHB frequency range it is used to
    stretch enough the USB response time to IN tokens, the USB turnaround time,
    so to compensate for the longer AHB read access latency to the Data FIFO */
 ...
 *            @arg USB_OTG_SPEED_HIGH_IN_FULL: High speed core in Full Speed
 * mode
 ...
 */
HAL_StatusTypeDef
USB_ActivateDedicatedEndpoint(const USB_OTG_GlobalTypeDef *USBx,
                              const USB_OTG_EPTypeDef *ep) {
    uint32_t USBx_BASE = (uint32_t)USBx;
    uint32_t epnum = (uint32_t)ep->num;
 ...
 */
HAL_StatusTypeDef
USB_DeactivateDedicatedEndpoint(const USB_OTG_GlobalTypeDef *USBx,
                                const USB_OTG_EPTypeDef *ep) {
    uint32_t USBx_BASE = (uint32_t)USBx;
    uint32_t epnum = (uint32_t)ep->num;
        ...
        if (ep->xfer_len == 0U) {
            ...
            if (epnum == 0U) {
                ...
                pktcnt = (uint16_t)((ep->xfer_len + ep->maxpacket - 1U) /
                                    ep->maxpacket);
                USBx_INEP(epnum)->DIEPTSIZ |=
                    (USB_OTG_DIEPTSIZ_PKTCNT & ((uint32_t)pktcnt << 19));
...
HAL_StatusTypeDef USB_WritePacket(const USB_OTG_GlobalTypeDef *USBx,
                                  uint8_t *src, uint8_t ch_ep_num, uint16_t len,
                                  uint8_t dma) {
    uint32_t USBx_BASE = (uint32_t)USBx;
    uint8_t *pSrc = src;
    uint32_t count32b;
...
/**
 * @brief  USB_ReadDevAllOutEpInterrupt: return the USB device OUT endpoints
 * interrupt status
 * @param  USBx  Selected device
...
/**
 * @brief  USB_ReadDevAllInEpInterrupt: return the USB device IN endpoints
 * interrupt status
 * @param  USBx  Selected device
...
HAL_StatusTypeDef USB_HostInit(USB_OTG_GlobalTypeDef *USBx,
    ...
    if (USBx == USB_OTG_HS) {
        ...
        USBx->DIEPTXF0_HNPTXFSIZ =
            (uint32_t)(((0x100U << 16) & USB_OTG_NPTXFD) | 0x200U);
        USBx->HPTXFSIZ =
            (uint32_t)(((0xE0U << 16) & USB_OTG_HPTXFSIZ_PTXFD) | 0x300U);
    } else
...
HAL_StatusTypeDef USB_HC_Init(USB_OTG_GlobalTypeDef *USBx, uint8_t ch_num,
    ...
    USBx_HC((uint32_t)ch_num)->HCCHAR =
        ...
        ((uint32_t)mps & USB_OTG_HCCHAR_MPSIZ) | USB_OTG_HCCHAR_MC_0 |
...
HAL_StatusTypeDef USB_HC_StartXfer(USB_OTG_GlobalTypeDef *USBx,
    ...
    if (hc->do_ssplit == 1U) {
        ...
        /*
         * For IN channel HCTSIZ.XferSize is expected to be an integer multiple
         * of max_packet size.
         */
        if (hc->ep_is_in != 0U) {
            hc->XferSize = (uint32_t)num_packets * hc->max_packet;
    ...
    if (hc->do_ssplit == 1U) {
        ...
        USBx_HC((uint32_t)ch_num)->HCSPLT =
            ((uint32_t)hc->hub_addr << USB_OTG_HCSPLT_HUBADDR_Pos) |
            (uint32_t)hc->hub_port_nbr | USB_OTG_HCSPLT_SPLITEN;

        /* unmask ack & nyet for IN/OUT transactions */
        USBx_HC((uint32_t)ch_num)->HCINTMSK |=
            (USB_OTG_HCINTMSK_ACKM | USB_OTG_HCINTMSK_NYET);
...
HAL_StatusTypeDef USB_HC_Halt(const USB_OTG_GlobalTypeDef *USBx,
    ...
    /* In buffer DMA, Channel disable must not be programmed for non-split
       periodic channels. At the end of the next uframe/frame (in the worst
       case), the core generates a channel halted and disables the channel
       automatically. */
 ...
 */
HAL_StatusTypeDef
USB_DeActivateRemoteWakeup(const USB_OTG_GlobalTypeDef *USBx) {
    uint32_t USBx_BASE = (uint32_t)USBx;
