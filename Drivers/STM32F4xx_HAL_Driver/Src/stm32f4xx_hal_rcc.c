...
System busses clocks (SYSCLK, AHB, APB1 and APB2).
...
and/or RTC clock source.
         ...
         (#) HSE (high-speed external), 4 to 26 MHz crystal oscillator used
directly or through the PLL as System clock source. Can be used also as RTC
clock source.

         (#) LSE (low-speed external), 32 KHz oscillator used as RTC clock
source.

         (#) PLL (clocked by HSI or HSE), featuring two different output clocks:
           (++) The first output is used to generate the high speed system clock
(up to 168 MHz)
           (++) The second output is used to generate the clock for the USB OTG
FS (48 MHz), the random analog generator (<=48 MHz) and the SDIO (<= 48 MHz).
...
HSI and an interrupt is generated if enabled. The interrupt is linked to the
Cortex-M4 NMI (Non-Maskable Interrupt) exception vector.

         (#) MCO1 (microcontroller clock output), used to output HSI, LSE, HSE
or PLL clock (through a configurable prescaler) on PA8 pin.

         (#) MCO2 (microcontroller clock output), used to output HSE, PLL,
SYSCLK or PLLI2S clock (through a configurable prescaler) on PC9 pin.

    [..] System, AHB and APB busses clocks configuration
         (#) Several clock sources can be used to drive the System clock
(SYSCLK): HSI, HSE and PLL. The AHB clock (HCLK) is derived from System clock
through configurable prescaler and used to clock the CPU, memory and peripherals
...
function to retrieve the frequencies of these clocks.
         ...
         (#) For the STM32F405xx/07xx and STM32F415xx/17xx devices, the maximum
             frequency of the SYSCLK and HCLK is 168 MHz, PCLK2 84 MHz and PCLK1
42 MHz. Depending on the device voltage range, the maximum frequency should be
adapted accordingly (refer to the product datasheets for more details).

         (#) For the STM32F42xxx, STM32F43xxx, STM32F446xx, STM32F469xx and
STM32F479xx devices, the maximum frequency of the SYSCLK and HCLK is 180 MHz,
PCLK2 90 MHz and PCLK1 45 MHz. Depending on the device voltage range, the
maximum frequency should be adapted accordingly (refer to the product datasheets
for more details).

         (#) For the STM32F401xx, the maximum frequency of the SYSCLK and HCLK
is 84 MHz, PCLK2 84 MHz and PCLK1 42 MHz. Depending on the device voltage range,
the maximum frequency should be adapted accordingly (refer to the product
datasheets for more details).

         (#) For the STM32F41xxx, the maximum frequency of the SYSCLK and HCLK
is 100 MHz, PCLK2 100 MHz and PCLK1 50 MHz. Depending on the device voltage
range, the maximum frequency should be adapted accordingly (refer to the product
datasheets for more details).
...
/**
 * @brief  Initializes the CPU, AHB and APB busses clocks according to the
 ...
 * will occur when the clock source will be ready.
 *
...
HAL_RCC_ClockConfig(const RCC_ClkInitTypeDef *RCC_ClkInitStruct,
    ...
    if (FLatency > __HAL_FLASH_GET_LATENCY()) {
        ...
        access the Flash memory by reading the FLASH_ACR register */
...
/**
 * @brief  Selects the clock source to output on MCO1 pin(PA8) or on MCO2
 * pin(PC9).
 ...
 *            @arg RCC_MCO2SOURCE_SYSCLK: System clock (SYSCLK) selected as MCO2
 * source
 *            @arg RCC_MCO2SOURCE_PLLI2SCLK: PLLI2S clock selected as MCO2
 * source, available for all STM32F4 devices except STM32F410xx
 *            @arg RCC_MCO2SOURCE_I2SCLK: I2SCLK clock selected as MCO2 source,
 * available only for STM32F410Rx devices
...
void HAL_RCC_MCOConfig(uint32_t RCC_MCOx, uint32_t RCC_MCOSource,
    ...
    if (RCC_MCOx == RCC_MCO1) {
        ...
        /* Mask MCO1 and MCO1PRE[2:0] bits then Select MCO1 clock source and
         * prescaler */
        ...
        /* Mask MCO2 and MCO2PRE[2:0] bits then Select MCO2 clock source and
         * prescaler */
...
void HAL_RCC_GetClockConfig(RCC_ClkInitTypeDef *RCC_ClkInitStruct,
    ...
    RCC_ClkInitStruct->APB2CLKDivider =
        (uint32_t)((RCC->CFGR & RCC_CFGR_PPRE2) >> 3U);
