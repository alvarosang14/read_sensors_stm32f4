...
static void USBD_GetDescriptor(USBD_HandleTypeDef *pdev,
        ...
        default:
            ...
            if (pdev->pDesc->GetUserStrDescriptor != NULL) {
                pbuf = pdev->pDesc->GetUserStrDescriptor(
                    pdev->dev_speed, LOBYTE(req->wValue), &len);
            } else {
                USBD_CtlError(pdev, req);
                err++;
            }
    ...
    case USB_DESC_TYPE_DEVICE_QUALIFIER:
        if (pdev->dev_speed == USBD_SPEED_HIGH) {
            ...
            if ((uint8_t)(pdev->NumClasses) > 0U) {
                pbuf =
                    (uint8_t *)USBD_CMPSIT.GetDeviceQualifierDescriptor(&len);
            } else
#endif /* USE_USBD_COMPOSITE */
            {
                pbuf = (uint8_t *)pdev->pClass[0]->GetDeviceQualifierDescriptor(
                    &len);
            }
        } else {
            USBD_CtlError(pdev, req);
    ...
    case USB_DESC_TYPE_OTHER_SPEED_CONFIGURATION:
        if (pdev->dev_speed == USBD_SPEED_HIGH) {
            ...
            if ((uint8_t)(pdev->NumClasses) > 0U) {
                pbuf =
                    (uint8_t *)USBD_CMPSIT.GetOtherSpeedConfigDescriptor(&len);
            } else
#endif /* USE_USBD_COMPOSITE */
            {
                pbuf =
                    (uint8_t *)pdev->pClass[0]->GetOtherSpeedConfigDescriptor(
                        &len);
            }
